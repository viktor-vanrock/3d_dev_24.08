...
export function RegisterVerifyPage() {
  ...
  async function submit(event: React.FormEvent) { event.preventDefault(); const result = await verifyRegistration(email, code); if (!result.ok) return setError(result.error ?? { message: "Неверный или просроченный код." }); sessionStorage.removeItem("portal.registration.email"); navigate("/", "back"); window.location.reload(); }
