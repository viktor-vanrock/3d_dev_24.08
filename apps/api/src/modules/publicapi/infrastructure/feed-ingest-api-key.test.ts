...
describe("проверка feed_ingest API-ключа", () => {
  it("принимает только активный ключ с feed_ingest scope", async () => {
    ...
    await expect(createFeedIngestApiKeyVerifier(db as never).verify(secret)).resolves.toEqual({
      id: "key-1",
      userId: "vendor-bot-1",
      scope: "feed_ingest",
    });
  ...
  it.each([
    ...
    await expect(createFeedIngestApiKeyVerifier(db as never).verify(rawKey)).resolves.toBeNull();
  });

  it("передаёт в БД только SHA-256 и фильтрует active/feed_ingest", async () => {
    const db = database([row]);
    await createFeedIngestApiKeyVerifier(db as never).verify(secret);
