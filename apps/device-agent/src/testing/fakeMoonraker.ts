...
// Эмулированный Moonraker (карточка MF-391 шаг 1 явно допускает «реальный (или
// эмулированный) Moonraker» как «готово когда») — HTTP (`/access/oneshot_token`,
// `/server/files/upload`, `/printer/info`) + WS JSON-RPC (`/websocket`), ровно тот срез
// реального API, что используют MoonrakerDriver и коннекторы (`/printer/info` — тот же
// identity-check, что apps/web/src/park/ipcheck.ts и connector/snapmaker/snapmakerConnector.ts).
// Не пытается быть полным Moonraker — только контракт, который реально используется;
// расширять по мере роста драйвера/коннектора, не заранее.

export interface FakeMoonrakerOptions {
  apiKey?: string;
  objects?: string[];
  webcams?: Array<{ stream_url: string; service?: string }>;
  /** `result.hostname` в ответе `/printer/info` — единственный сегодня доступный сигнал модели. */
  hostname?: string;
  /** false → `/printer/info` отвечает без `result` (эмулирует wrong-device/не-Moonraker хост). */
...
export interface FakeMoonraker {
   ...
   *  push-путь driver.onStatusUpdate без polling. */
...
export async function startFakeMoonraker(options: FakeMoonrakerOptions = {}): Promise<FakeMoonraker> {
  ...
  async function handleHttp(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (req.method === "GET" && url.pathname === "/printer/info") {
      if (options.respondToIdentity === false) {
        res.setHeader("content-type", "application/json");
        res.end(JSON.stringify({ error: "not a moonraker host" }));
        return;
      }
      res.setHeader("content-type", "application/json");
      res.end(JSON.stringify({ result: { hostname: options.hostname ?? "snapmaker-u1", state: "ready" } }));
      return;
    }
    ...
    if (req.method === "GET" && url.pathname.startsWith("/server/files/")) {
      if (options.apiKey && req.headers["x-api-key"] !== options.apiKey) {
        res.writeHead(401).end(JSON.stringify({ error: "unauthorized" }));
        return;
      }
...
// которое реально шлёт MoonrakerDriver.uploadGcode (FormData + Blob). Не претендует на полный
